...
// ─── PUNTO 4: Análisis en dos fases ───────────────────────────────────────────

/**
 * FASE 1: Con todos los datos de mercado, Claude identifica los 5 mejores candidatos
 * y el contexto general. Respuesta rápida y amplia.
 */
...
/**
 * FASE 2: Con los candidatos de la Fase 1 + datos técnicos específicos de esos tickers,
 * Claude hace el análisis fino con niveles exactos de entrada, stop y target.
 */
...
/**
 * Entry point principal — orquesta las dos fases
 */
async function analyzeWithClaude({ marketData, news, sentiment, economicCalendar, technicals, earningsRisk, delayMinutes = 30, pastAnalyses = [] }) {
  ...
  if (!phase1 || phase1.error) {
    console.error('[Claude] Phase 1 failed:', phase1?.error);
    throw new Error('Phase 1 analysis failed: ' + (phase1?.error || 'unknown'));
  }

  // Si el mercado no es operable, saltar fase 2
  ...
  // FASE 2: solo para los candidatos seleccionados en fase 1
  const candidates = phase1.candidates ?? [];
  const candidateTechnicals = {};
  candidates.forEach(c => {
    if (technicals[c.ticker]) candidateTechnicals[c.ticker] = technicals[c.ticker];
  });

  const phase2 = await phaseTwo({ phase1Result: phase1, technicals: candidateTechnicals, earningsRisk, pastAnalyses });

  if (!phase2 || phase2.error) {
    console.warn('[Claude] Phase 2 failed, using Phase 1 context only.');
  }

  // PUNTO 8: Validar y corregir output
  const validatedRecs = validateAndFixRecommendations(phase2?.recommendations ?? []);

  // Aplicar advertencias de earnings sobre las recomendaciones
...
// ─── PUNTO 8: Validación del output de Claude ──────────────────────────────────
...
function validateAndFixRecommendations(recs) {
  ...
  return recs.map(rec => {
    ...
    // 1. Target debe ser MAYOR que la entrada (para compras)
    if (rec.action === 'COMPRAR' && rec.target <= rec.entryHigh) {
      issues.push(`Target $${rec.target} ≤ entrada $${rec.entryHigh} — inválido`);
      fixed.target = parseFloat((rec.entryHigh * 1.05).toFixed(2)); // mínimo 5%
    }

    // 2. Stop loss debe ser MENOR que la entrada (para compras)
    ...
    // 3. Recalcular R/R para verificar consistencia
    if (fixed.target && fixed.stopLoss && entry) {
      ...
      // Si el R/R declarado difiere >30% del calculado, corregir
      ...
      // Descartar si el R/R calculado es menor a 1.5:1
      if (rrCalculated < 1.5) {
        issues.push(`R/R calculado (${rrCalculated.toFixed(1)}:1) < 1.5:1 mínimo — descartado`);
    ...
    return fixed;
  }).filter(Boolean); // eliminar los descartados
}
...
function applyEarningsWarnings(recs, earningsRisk) {
  ...
  return recs.map(rec => {
    ...
    const warning = earnings.message;
    const existingWarnings = rec.riskWarnings ?? [];
    ...
    return {
      ...
      riskWarnings: [warning, ...existingWarnings],
      // Si reporta HOY, bajar la confianza a MEDIA como mínimo
...
function buildMemoryContext(pastAnalyses) {
  ...
  const recent = pastAnalyses.slice(0, 3); // últimos 3 análisis

  const lines = recent.map((a, i) => {
    const date = new Date(a.generatedAt).toLocaleDateString('en-US', {
...
Usá este contexto para:
- Identificar tendencias que persisten vs que se agotaron
- Evaluar si las recomendaciones anteriores se materializaron o fallaron
- Ajustar el sesgo considerando la secuencia reciente de días
`;
}
...
function buildPhaseOnePrompt({ marketData, news, sentiment, economicCalendar, delayMinutes }) {
  ...
  const { indices, vix, sectors, gainers, losers, unusualVolume, gapAnalysis, preMarket, breadth } = marketData;
  ...
  const sectorsSection = sectors.map(s =>
    `  ${s.name.padEnd(18)}: ${fmtPct(s.changePct)}`
  ).join('\n');

  const gainersSection = gainers.slice(0, 8).map(g =>
    `  ${g.symbol.padEnd(8)} ${fmtPct(g.changePct).padStart(8)} | $${g.price?.toFixed(2)} | Vol: ${fmtVol(g.volume)}`
  ).join('\n');

  const unusualVolSection = unusualVolume.slice(0, 8).map(g =>
    `  ${g.symbol.padEnd(8)} ${fmtPct(g.changePct).padStart(8)} | Vol ratio: ${g.volumeRatio}x`
  ).join('\n');

  const newsSection = (news.headlines ?? []).slice(0, 12).map(n =>
    `  [${n.sentiment}] ${n.title} (${n.source})`
  ).join('\n');

  const econSection = economicCalendar?.events?.length
    ? economicCalendar.events.slice(0, 8).map(e =>
        `  [${e.impact}] ${e.event} ${e.time} — ${e.actual != null ? `Actual: ${e.actual}` : 'Pendiente'} ${e.surprise ? `(${e.surprise})` : ''}`
      ).join('\n')
    : '  Sin eventos económicos relevantes hoy';

  const preMarketSummary = preMarket
    ? `Pre-market consensus: ${preMarket.consensus} | Avg: ${fmtPct(preMarket.avgPreMarketPct)}`
    : 'Pre-market: No disponible';

  const breadthSummary = breadth
    ? `Amplitud: ${breadth.breadthSignal} | Sectores en verde: ${breadth.sectorBreadth?.pct ?? 'N/A'}% | Vol ratio promedio: ${breadth.avgVolumeRatio}x`
    : 'Amplitud: No disponible';

  const vixSection = vix
    ? `VIX: ${vix.value?.toFixed(2)} (${fmtPct(vix.changePct)}) → ${vix.interpretation}`
    : 'VIX: No disponible';

  const fgSection = sentiment.score != null
    ? `Fear & Greed: ${sentiment.score}/100 — ${sentiment.rating} | Tendencia: ${sentiment.trend}`
...
function buildPhaseTwoPrompt({ phase1Result, technicals, earningsRisk, pastAnalyses }) {
  ...
  // Formatear técnicos por candidato
...
Usá TODOS los datos técnicos para determinar:
1. Niveles EXACTOS de entrada (rango bid-ask realista basado en soporte/resistencia y VWAP)
2. Stop loss basado en ATR o soporte técnico clave (no arbitrario)
3. Target basado en resistencia técnica o relación R/R mínimo 2:1
4. Si un candidato de Fase 1 NO tiene buenos técnicos, EXCLUILO de las recomendaciones finales
5. Máximo 3 recomendaciones — calidad sobre cantidad
  ...
  "recommendations": [
    {
      ...
      "stopLoss": 0.00,
      "rrRatio": "X.X:1",
      "potentialGainPct": "X.X%",
      "potentialLossPct": "X.X%",
      "confidence": "ALTA|MEDIA",
      "timeHorizon": "INTRADAY|1-3 DÍAS",
      "catalyst": "Catalizador principal",
      "technicalSetup": "Setup técnico detallado con niveles",
...
// Exportar fase 1 por separado para que el orquestador pueda
// obtener los candidatos ANTES de buscar sus técnicos
