...
const FINANCIAL_SOURCES = ['bloomberg','the-wall-street-journal','financial-times','cnbc','reuters','business-insider','fortune','the-economist'].join(',');
  ...
  try {
    ...
    const [response, queryResponse] = await Promise.all([
      withRetry(() => axios.get(`${NEWS_API_BASE}/top-headlines`, {
        params: { sources: FINANCIAL_SOURCES, apiKey, pageSize: 20, language: 'en' }, timeout: 10000,
      }), { label: 'news-headlines', retries: 2 }),
      withRetry(() => axios.get(`${NEWS_API_BASE}/everything`, {
        params: { q: 'stock market OR earnings OR Federal Reserve OR "Wall Street"', language: 'en', sortBy: 'publishedAt', from: since, pageSize: 15, apiKey }, timeout: 10000,
    ...
    const headlines = allArticles.slice(0, 20).map(a => ({
      title: a.title, source: a.source?.name ?? 'Unknown', publishedAt: a.publishedAt,
      description: a.description ? a.description.slice(0, 150) : '', sentiment: guessSentiment(a.title),
    }));
...
function guessSentiment(title) {
  ...
  const bullish = ['surge','rally','gain','rise','beat','record','high','growth','strong','boost','jump','soar','upgrade','buy','bullish','recovery'];
  const bearish = ['fall','drop','decline','crash','plunge','miss','low','weak','concern','worry','fear','sell','downgrade','bearish','recession','cut','loss'];
